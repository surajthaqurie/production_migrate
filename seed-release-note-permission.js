/**
 * Backfills RELEASE_NOTE permission into existing admin roles in the SaaS database.
 * RELEASE_NOTE is a SaaS-admin-only resource (no tenant-side equivalent), so unlike
 ...
 *
 * Usage: node scripts/seed-release-note-permission.js
...
const RESOURCE = "RELEASE_NOTE";

// Mirrors src/background/queues/db-seed-queue/seed/permission-seed/*.constant.ts —
// SUPER_ADMIN and ADMIN get the actions used by active routes.
// SUPPORT_USER, READ_ONLY, and SEO retain view-only access.
const ROLE_ACTIONS = {
...
async function main() {
  ...
  try {
    ...
    // Check which of the target roles already have a RELEASE_NOTE permission set
