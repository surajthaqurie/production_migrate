/**
 * Backfills API_INTEGRATION permission into the admin and super-admin roles
 * of all existing tenant databases. Safe to run multiple times — uses ON CONFLICT DO NOTHING.
 *
 * Usage: node scripts/seed-api-integration-permission.js
...
const RESOURCE = "API_INTEGRATION";
const ACTIONS = ["VIEW", "CREATE", "ENABLE_DISABLE"];
const TARGET_SLUGS = ["admin", "super-admin"];

async function seedApiIntegrationPermission(rootUrl, databaseName) {
  ...
  try {
    ...
    // Check which of the target roles already have an API_INTEGRATION permission set
...
async function main() {
  ...
  try {
    ...
    for (const tenant of tenants) {
      ...
      try {
        await seedApiIntegrationPermission(databaseUrl, tenant.databaseName);
        succeeded++;
      } catch (error) {
