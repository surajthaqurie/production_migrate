/**
 * Backfills SUPPORT_TICKET permission into all existing tenant databases.
 * Safe to run multiple times — uses ON CONFLICT DO NOTHING.
 *
 * Usage: node scripts/seed-support-ticket-permission.js
...
// Actions applied to every role, regardless of name
const DEFAULT_ACTIONS = ["VIEW", "CREATE"];

async function seedSupportTicketPermission(rootUrl, databaseName) {
  ...
  try {
    ...
    const superAdminRole = await db.selectFrom("user_roles").select("id").where("slug", "=", "super-admin").executeTakeFirst();
    ...
    // Every role in the tenant gets SUPPORT_TICKET permission if it doesn't already have it
    const roles = await db.selectFrom("user_roles").select(["id", "name"]).execute();
    ...
    // Check which roles already have a SUPPORT_TICKET permission set
    const existing = await db
      .selectFrom("permission_sets")
      .select(["id", "roleId", "actions"])
      .where("resource", "=", "SUPPORT_TICKET")
      .where(
        "roleId",
        "in",
    ...
    const toUpdate = existing
      .map((row) => ({ row, mergedActions: Array.from(new Set([...row.actions, ...DEFAULT_ACTIONS])) }))
...
async function main() {
  ...
  try {
    ...
    for (const tenant of tenants) {
      ...
      try {
        await seedSupportTicketPermission(databaseUrl, tenant.databaseName);
        succeeded++;
      } catch (error) {
