...
async function up(db) {
    ...
    await db.schema.createIndex("user_audit_logs_branch_idx").on("user_audit_logs").column("branchId").execute();
}
async function down(db) {
    await db.schema.dropIndex("user_audit_logs_user_idx").execute();
    await db.schema.dropIndex("user_audit_logs_branch_idx").execute();
    await db.schema.dropTable("user_audit_logs").ifExists().execute();
}
//# sourceMappingURL=1785232025558_user_audit_logs.js.map